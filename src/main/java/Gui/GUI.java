...
import Word_classes.*;
import SQL.*;
import Excel_classes.ImportExcelFile;
import OtherClasses.*;
...
public final class GUI extends JPanel {
    ...
    private final CheckBoxDefaultTableModel model = new CheckBoxDefaultTableModel(COLUMNS, 0);
    ...
    //static String samord = prefs.get("samordString", "");
    private static final String chef = "x y";
    ...
    private final String[] GUIabsenceOpt = {" ", "S", "A", "V", "Ö", "P", "AF", "0,15", "0,30", "0,45", "1", "1,15", "1,30", "1,45", "2", "2,15", "2,30", "2,45",
        "3", "3,15", "3,30", "3,45", "4", "4,15", "4,30", "4,45", "5", "5,15", "5,30", "5,45", "6", "6,15", "6,30", "6,45",
        "7", "7,15", "7,30", "7,45", "8"};

    private final String[] changeAbsenceOpt = {"Återställ", "x", "h", "d", "S", "A", "V", "Ö", "P", "AF", "SFI", "SO", "0,15", "0,30", "0,45", "1", "1,15", "1,30",
        "1,45", "2", "2,15", "2,30", "2,45", "3", "3,15", "3,30", "3,45", "4", "4,15", "4,30", "4,45", "5",
        "5,15", "5,30", "5,45", "6", "6,15", "6,30", "6,45", "7", "7,15", "7,30", "7,45", "8"};

    private final String[] scheduleOpt = {"HELTID", "EM", "FM", "SFI-EM", "FM-SFI", "SFI", "SO-EM", "FM-SO",
    ...
    private final String[] ScheduleOptTipText = {"Heltid hos oss (100%)", "Bara på EM hos oss (50%)",
        ...
        selectDate.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (raportMonth.length() > 0) {
                    raportYear = text.getText().charAt(0) + "" + text.getText().charAt(1) + "" + text.getText().charAt(2)
                            + "" + text.getText().charAt(3);
                    raportMonth = text.getText().charAt(5) + "" + text.getText().charAt(6);
                } else {
        ...
        saveInfo.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                if (nameF.getText().isEmpty() == false && timeF.getText().isEmpty() == false
                    ...
                    if (timeF.getText().trim().equalsIgnoreCase("fm") || timeF.getText().trim().equalsIgnoreCase("em")
                            || timeF.getText().trim().equalsIgnoreCase("heltid") || timeF.getText().trim().equalsIgnoreCase("fm/em")
                            || timeF.getText().trim().equalsIgnoreCase("deltid") || timeF.getText().trim().equalsIgnoreCase("individuellt schema")) {
                        if ((isValidFormat("yyyy-MM-dd", startF.getText().trim()) && (isValidFormat("yyyy-MM-dd", endF.getText().trim())))) {
                            ...
                            if (caseF.getText().trim().length() < 8 || caseF.getText().trim().length() > 8) {
                                int reply = JOptionPane.showConfirmDialog(null, "Ärendenumret brukar vara åtta siffror långt.\n"
                                        + "Skrev du rätt ärendenummer?", "Säkerhetsfråga", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE);
                                if (reply == JOptionPane.YES_OPTION) {
                                    if (timeF.getText().trim().equalsIgnoreCase("deltid") && !GUI.db.tableExist(caseF.getText().trim())) {
                                        JOptionPane.showMessageDialog(null, "Du valde ett deltidsschema.\n"
                                                + "Om deltagaren är här på FM, EM och ibland Heltid så ska man skapa\n"
                                                + "ett individuellt schema istället ('Individuellt schema' - knappen)", "Meddelande", JOptionPane.DEFAULT_OPTION);
                                    }
                                ...
                                if ((timeF.getText().trim().equalsIgnoreCase("deltid") || timeF.getText().trim().equalsIgnoreCase("fm/em")) && !GUI.db.tableExist(caseF.getText().trim())) {
                                    JOptionPane.showMessageDialog(null, "Du valde ett deltidsschema.\n"
                                            + "Om deltagaren är här på FM, EM och ibland Heltid så ska man skapa\n"
                                            + "ett individuellt schema istället ('Individuellt schema' - knappen)", "Meddelande", JOptionPane.DEFAULT_OPTION);
                                }
